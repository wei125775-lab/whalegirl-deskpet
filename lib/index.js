/**
 * @wei125775-lab/dsh-pet-whalegirl
 *
...
function releasePet() {
  try {
    ...
    if (packaged === undefined) {
      console.warn('[dsh-pet-whalegirl] 包内 pet/pet.json 读不出来，跳过释放')
      return
    }
    if (petVersion(target) === packaged) return
    rmSync(target, { recursive: true, force: true })
    mkdirSync(dirname(target), { recursive: true })
    cpSync(source, target, { recursive: true })
    console.log('[dsh-pet-whalegirl] 已释放宠物 ' + PET_ID + ' ' + packaged + ' 到 ' + target)
  } catch (error) {
    console.warn('[dsh-pet-whalegirl] 释放宠物失败：' + (error && error.message ? error.message : String(error)))
  }
}
...
export const name = '@wei125775-lab/dsh-pet-whalegirl'
export const inject = []
