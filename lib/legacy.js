/**
 * legacy 路径：把自己挂在 @linxin666/dsh-pet 上（释放素材 + 打相位白名单补丁）。
 *
 * 什么时候走这条路：profile 里装了 dsh-pet 的时候。它有三种形态 ——
 *   1. 素材必须"释放"到 `$DSH_HOME/pets/`，因为 dsh-pet 只从那儿扫；
 *   2. 「看鲸鱼」必须改它的 ACTIVITY_PHASES 白名单（fail-closed，不认识的相位名
 *      会让整只宠物被静默丢弃）；
 *   3. 因为它的相位是全局共享的，子代理事件会抢显示，所以演出期间要把服务实例上的
 *      applyActivity 包一层、吞掉子代理的调用。
 *
 * 不装 dsh-pet 的 profile（官方桌面版）走 lib/engine/ 那套自研引擎，这三条都不需要。
 *
 * ---
 *
...
/**
 * `pet.json` 的指纹归一化：把 whale-* 相位去掉再序列化。
 *
 * `syncWhalePhases()` 会在 dsh-pet 没打补丁时把这三个相位从**部署的那份**里摘掉
 * （fail-closed 校验，留着会让整只宠物被拒）—— 那是我们自己写的降级，不是内容变了。
 * 直接比字节的话指纹永远对不上：实测复现过，降级态下每次启动都整目录重拷一遍
 * （64MB×2，还阻塞启动）。拿不到 JSON 就原样返回。
 */
function normalizeManifest(content) {
  try {
    const raw = JSON.parse(content.toString('utf8'))
    const phases = raw?.frames2d?.phases
    if (phases === null || typeof phases !== 'object') return content
    for (const phase of WHALE_PHASES) delete phases[phase]
...
function petDigest(petDir) {
  try {
    ...
    for (const rel of files) {
      ...
      if (rel === 'pet.json') {
        // 单独一条：内容和"大小"都得用归一化后的 —— 部署那份被摘过相位，两样都不同。
        const normalized = normalizeManifest(readFileSync(abs))
        hash.update(rel + ':' + normalized.length + '\n')
...
/**
 * 包内那份素材够不够 dsh-pet 认。返回问题描述；没问题返回 undefined。
 *
 * **必须在拷贝之前验**：覆盖是"整目录重拷"，包里不完整就会用坏的顶掉好的部署副本。
 * 而缺的要是被 `phases.idle` 引用的那条轨的帧，dsh-pet 的 `resolveFrames2dEntry` 会判
 * error → `scanPetDir` 的 `if (!verdict.ok) continue` → **整只宠物从注册表消失**
 * （不是"显示不全"，是没了；若她本是选中项，界面会回落到内置那只）。2026-09-19 踩过：
 * 加了海豚素材却只拷了 pet.json 没拷 frames/，54 帧凭空消失。
 *
 * `missingFrames()` 自己吞异常（读不出来就返回空数组），所以这里单独把 pet.json 的
 * 解析和"待机轨有没有声明帧"两道先判掉，不能指望它。
 */
function sourceProblem(petDir) {
  let raw
  try {
    raw = JSON.parse(readFileSync(join(petDir, 'pet.json'), 'utf8'))
  } catch (error) {
    return 'pet.json 读不出来（' + (error && error.message ? error.message : String(error)) + '）'
  }
  const frames2d = raw?.frames2d
  if (frames2d === undefined || frames2d.tracks === null || typeof frames2d.tracks !== 'object') {
    return '没有 frames2d.tracks'
  }
  const idle = typeof frames2d.phases?.idle === 'string' ? frames2d.phases.idle : 'idle'
  const idleFrames = frames2d.tracks[idle]?.frames
  if (!Array.isArray(idleFrames) || idleFrames.length === 0) {
    return '待机轨 ' + idle + ' 没有声明帧（dsh-pet 会因此丢掉整只宠物）'
  }
  const missing = missingFrames(petDir)
  if (missing.length > 0) {
    return missing.length + ' 个引用帧在磁盘上找不到（' + missing.slice(0, 6).join(', ') + '…）'
  }
  return undefined
}
...
function releasePet() {
  try {
    ...
    if (problem !== undefined) {
      console.warn(
        '[whalegirl-deskpet] ⚠ 包内素材不完整（' + problem + '），**这次不动部署目录**，继续用已装那份。' +
        '多半是拿到的包缺文件（下载/解压漏了 pet/frames/）：换一份完整的再启动。',
      )
      return
    }
    ...
    // 释放是"整目录重拷"，拷一半（磁盘满、杀软锁文件）也会走到这里，而且不报错 ——
    // 表现只是动作缺帧/画面空白。包内那份拷之前已经验过，所以这时候的缺失只能是拷贝没拷全。
    const missing = missingFrames(target)
    if (missing.length > 0) {
      console.warn(
        '[whalegirl-deskpet] ⚠ 释放后仍有 ' + missing.length + ' 个引用帧找不到 —— 包内那份是完好的（拷之前验过），' +
        '所以是这次拷贝没拷全。删掉 ' + target + ' 再重启会重新释放：' +
...
/**
 * 顶层要做的那两件事，单独抽出来给入口调用。
 *
 * **必须是模块顶层级别的时机，不能挪进 apply()**：宠物注册表是 dsh-pet 在它自己的
 * apply 期间用 loadPetRegistry 扫目录建的，而顶层副作用发生在整棵插件树的 apply
 * 之前 —— 装完首次启动才扫得到。写进 apply 就要看两个插件的 apply 谁先谁后了。
 *
 * 入口只在**确认走 legacy 路径时**才调它，所以 native 模式下素材不会被白白拷一份。
 */
...
function startWhaleWatch(ctx) {
  ...
  if (service === undefined || typeof service.applyActivity !== 'function') {
    // 不能静默：这条路径失守的表现只是"子代理每次都把鲸鱼打断重播"，日志里什么都没有。
    // （`applyActivity` 在 dsh-pet 里是 TS private，靠编译后 private 被擦除才存在于原型上 ——
    //   它哪天改成真私有 `#applyActivity` 或换名，就会走到这里。）
    console.warn('[whalegirl-pet] 拿不到 pet 服务的 applyActivity，看鲸鱼期间的打断抑制未生效（dsh-pet 可能改了实现）')
    return
  }
  ...
  ctx.on('session/event', (session, event) => {
    ...
    show.lastSeen = Date.now()
    // 只认 `turn/end`。原来这里还写着 `session/end` —— dsh 没有这个事件名
    // （真名是 `session/end-seed`，语义是 fork 前缀标记，不是会话结束），那样是死代码；
    // 会话结束由下面的 `session/disposed` 负责。
  ...
  ctx.on('session/disposed', (session) => {
    ...
    // 收场判据要和 `session/event` 那处对齐（`!== 'off' && !== 'out'`）：只认 'loop' 的话，
    // 会话在**入场那段**被销毁（取消/删会话/重载）时没人收场，入场定时器到点自己走进
    // loop 并停在那儿，演出最长挂 5 分钟，期间主相位全被冻住。
    if (show.active.size === 0 && show.state !== 'off' && show.state !== 'out') end()
  })
