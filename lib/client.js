...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function createPlayer(container, def) {
      ...
      let drawToken = 0
      let lastDrawnUrl
      /** 本相位里已经认过的**一次性**轨（见 follow）。 */
      let honoredTrack
      /** 上次跟随的相位，用来判"这是不是新的一次动作"。 */
      let syncedPhase
      ...
      return {
        /** 相位（或服务端让播的轨）变了时调用。 */
        syncToPhase(trackId, phase) {
          if (disposed) return
          if (override !== undefined) return
          if (typeof phase === 'string' && phase !== syncedPhase) {
            syncedPhase = phase
    ...
    function mount(ctx) {
      ...
      const pollNow = () => {
        ...
        void api('/api/whalegirl/state').then((next) => {
          ...
          if (player !== null && typeof next.track === 'string') player.syncToPhase(next.track, next.phase)
        }).catch(() => { /* 下个 tick 再试 */ })
      }
