...
补丁还顺手修了一个能把宠物**点死**的 bug：窗口尺寸或位置一变（换动作、缩放、拖拽之后），角色相对光标的位置就跟着变，而"鼠标在不在她身上"这个判断只在鼠标移动时才重算——鼠标没动时旧结论一直留着，窗口就卡在穿透状态。实测卡住以后连 mousemove 都收不到，用户怎么移鼠标都点不到她，只能重启。现在改成主进程每秒心跳推一次光标位置过去重算，不再依赖 Electron 的 forward 转发。
